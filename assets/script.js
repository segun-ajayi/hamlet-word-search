    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ...
          score++;
          $('#score').text(`${score}/${wordList.length+score}`)
        }
...
$(function () {
  const start = (e = null) => {
    ...
    var words = ['Akanbi', 'Waraosin', 'MogajiAre', 'Maya', 'OkoErin', 'Malete',
      ...
      'Okaka', 'Marafa', 'Alanamu', 'Ogidi', 'Zango', 'Jebba', 'Ogele', 'Afon', 'Lanwa',
      'Oloru', 'Yowere', 'Ibagun'];
  ...
  const login = (e = null) => {
    ...
    const pin = document.querySelector('#loginInput').value;
    const error = document.querySelector('#errorSpan');
    if (!pin) {
      error.textContent = 'Please enter an Hamlet Code!';
      return;
    }
    if (pin.length > 6 || pin.length < 6) {
      error.textContent = 'Please enter a valid Hamlet Code!';
      return
    }
    error.textContent = '';
  ...
  if('') {
    ...
    const div = document.querySelector('#puzzle');
